...
objs = [
    ...
    {'height': 2.5, 'width': 2.25, 'position': [1, 9.5], 're_centered_position': [2.125, 10.75]},
...
cartReturns = [2, 18.5]
basketReturns = [4, 18.5]
registerReturns_1 = [1, 4.5]
registerReturns_2 = [1, 9.5]
print(shopping_list)
...
if len(shopping_list) > 0:
    ...
    for item in shopping_list:
        ...
        path  = player.astar((player.player['position'][0], player.player['position'][1]),
                              (item_pos[0] + offset, item_pos[1]), objs, 20, 25)
        if path == None:
            continue
    ...
    path = player.astar((player.player['position'][0], player.player['position'][1]),
                        (registerReturns_1[0], registerReturns_1[1]), objs, 20, 25)
    if path == None:
        path = player.astar((player.player['position'][0], player.player['position'][1]),
                            (registerReturns_2[0], registerReturns_2[1]), objs, 20, 25)
    if path == None:
        print("no path to register")
    
    player.perform_actions(player.from_path_to_actions(path))
    player.face_item(registerReturns_1[0], registerReturns_1[1])
    player.step('INTERACT')
    print(player.obs)
